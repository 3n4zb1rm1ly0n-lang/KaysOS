...
function missingHint(
    ...
    if (msg.includes('blocked_amount')) return 'add_personal_income_blocked_amount.sql';
    return undefined;
}
...
export async function fetchDashboardFinance(
    ...
    if (incRes.error || expRes.error) {
        ...
        const blocked = incomes.reduce(
            (a, r) => a + incomeBlocked(r.amount, r.blocked_amount),
            0
        );
        const expense = expenses.reduce((a, r) => a + r.amount, 0);
        const usable = incomeUsable(income, blocked);
        personal = {
            ok: true,
            income,
            blocked,
