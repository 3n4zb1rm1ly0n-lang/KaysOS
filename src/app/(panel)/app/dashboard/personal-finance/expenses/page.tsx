...
export default function PersonalExpensesPage() {
    ...
    const load = useCallback(async (y: number, m: number) => {
        ...
        if (!incRes.error) {
            ...
            for (const r of incRes.data ?? []) {
                ...
                gross += amt;
                blocked += Math.min(parseMoney(r.withheld_amount), amt);
            }
            setIncomeTotal(gross);
            setBlockedTotal(blocked);
        } else if (incRes.error.message.includes('withheld_')) {
            setError(
                'Haciz bloke alanı eksik. Supabase’te create_personal_budget_savings.sql çalıştırın.'
            );
            setIncomeTotal(0);
            setBlockedTotal(0);
        } else {
    ...
    const remaining = incomeNetCash(incomeTotal, blockedTotal) - expenseTotal;
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                ...
                <div className="rounded-lg border border-primary/30 bg-primary/5 px-4 py-3">
                    ...
                    <p className="text-[11px] text-muted-foreground mt-0.5 tabular-nums">
                        Kullanılabilir gelir {fmtMoney(incomeNetCash(incomeTotal, blockedTotal))}
                    </p>
                </div>
            </div>
